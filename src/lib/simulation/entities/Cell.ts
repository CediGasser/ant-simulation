...
import Parameters from '../environment/SimulationParameters.svelte';
...
export default class Cell extends BaseEntity {
	...
	public render(p5: p5): void {
		if (this.fDuration == 0) {
			p5.fill(
				Colors.FIELD_COLOR[this.antType][0],
				Colors.FIELD_COLOR[this.antType][1],
				this.setBrightnessValue(Colors.FIELD_COLOR[this.antType][2])
			);
		} else {
	...
	private setCellsDistance(steps: number, property: string): void {
		...
		this.world.adjPos[this.position.x][this.position.y].forEach((position: p5.Vector) => {
			if (position.x == this.position.x || position.y == this.position.y) {
				(this.world.grid[position.x][position.y] as Cell).setDistance(steps + 1, property);
			} else {
				(this.world.grid[position.x][position.y] as Cell).setDistance(steps + 2, property);
			}
		}, this);
	}
